...
from .constants import OPENAI_DATASET_MEAN, OPENAI_DATASET_STD, INCEPTION_MEAN, INCEPTION_STD, RESCALE_FACTOR
...
class Rescale(torch.nn.Module):
    """Rescale the tensor by a factor."""
    ...
    def forward(self, x):
        ...
        if self.factor is not None:
            x=np.array(x)
            rescaled_image = x.astype(np.float64) * self.factor
            rescaled_image = rescaled_image.astype(np.float32)
            return rescaled_image  # 使用普通乘法而不是原地操作
        return x
...
def image_transform(
    ...
    if rescale_factor is not None:
        rescale_transform = Rescale(rescale_factor)
    else:
        rescale_transform =lambda x: x
