...
def resize_patch_pos_embed(state_dict, model):
    if 'visual.trunk.pos_embed' in state_dict:
        key= 'visual.trunk.pos_embed'
        model_patch_pos_embed=model.visual.trunk.pos_embed
    
    old_token_embed=state_dict.get(key)
    old_token_num=old_token_embed.shape[1]

    if model_patch_pos_embed.shape[1]!=old_token_num:
        # add_token_embed=model_patch_pos_embed[:,old_token_num:,:]
        # add_token_embed = add_token_embed.to(old_token_embed.device,dtype=old_token_embed.dtype)
        # new_token_embed=torch.cat([old_token_embed,add_token_embed],dim=1)
        new_token_embed=old_token_embed[:,:256,:]
